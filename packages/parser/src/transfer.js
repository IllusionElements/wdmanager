...
const ext = keys.get("extension");
const mainFile = path.join(process.cwd(), DIR, keys.get("main"));
const output = path.join(mainFile, keys.get("output"));
const matcher = new RegExp(process.env.MATCH.concat(ext), "i");
const copy = async (src, dest, flags) => {
    try {
        await fsPromise.copyFile(src, dest, flags);
    }
    catch (e) {
        await fsPromise.appendFile("./error", `${e.message}
    `);
    }
};
for (const file of fs.readdirSync(mainFile)) {
    if (file.match(matcher)) {
        ...
        fsPromise.copyFile(src, out, fs.constants.COPYFILE_EXCL).catch(e => fsPromise.appendFile("./error", `${e.message}
      `));
    }
}
