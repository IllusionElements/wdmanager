...
// Tier Resolver
export const Dragon = {
    rarity: pick("defaultRarity", (rarity, _, { dragons: { service } }) => service.rarity.get(rarity)),
    incubatorLevel: ({ incubationBuildingLevelRequirement }) => incubationBuildingLevelRequirement,
    // levels: (dragon: IDragon) => { },
    breedableLevel: ({ minDragonLevelForBreeding }) => minDragonLevelForBreeding,
    breedingCaveLevel: ({ minBreedingCaveBuildingLevelToBreed }) => minBreedingCaveBuildingLevelToBreed,
    isDivine: pick("isEvolveDragon", Boolean),
    frags: pick("numberOfFragmentsNeeded", Number),
    class: pick("dragonClass"),
    title: pick("dragonType"),
    defaultIconFileName: ({ defaultIconFilename }) => `${cdn}${defaultIconFilename}`,
